$(function () {
    ...
    $('#header .lnb .lnb-search-key .search-foot .btn-search-close').on('click',function (e) {
        e.preventDefault();
        e.stopPropagation();
        $('#header .lnb .lnb-search-key').hide();
    });
    $('#header .lnb .lnb-search-key .search-tab-wrap > a').on('click',function (e) {
        e.preventDefault();
        e.stopPropagation();
        $('#header .lnb .lnb-search-key').hide();
        if($(this).index() == 1) {
    ...
    if($('#header .nav').size() > 0) {
        var fixedOffset = $('#header .nav').offset().top + $('#header .nav').height() + 100;
    ...
    $('.recently-goods-wrap .btn-prev').on('click',function () {
        $('#quick-slide-wrap').trigger('prev.owl.carousel', [300]);
    });
    $('.recently-goods-wrap .btn-next').on('click',function () {
        $('#quick-slide-wrap').trigger('next.owl.carousel');
    });
    ...
    $('.accordion .accordion-t .btn-accordion').on('click',function (e) {
        ...
        if(!$(this).closest('.accordion').hasClass('toggle')) {
            ...
            $('.accordion').find('.accordion-t').not($(this).closest('.accordion-t')).removeClass('on');
            $(this).closest('.accordion-t').toggleClass('on');
        }
    });
